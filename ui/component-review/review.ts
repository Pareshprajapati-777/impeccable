...
export function mountComponentReview(host: HTMLElement, packet: ReviewPacket, options: {
  ...
  let finished = !summarize(packet,draft).pending;
  ...
  function render() {
    ...
    on('submit',async()=>{if(uncommitted||sending||submitted)return;sending=true;error='';render();try{await options.onSubmit(submission(packet,draft));submitted=true;finished=true;inventoryFilter='reviewed';}catch(e){error=e instanceof Error?e.message:'Could not save. Try again.';}finally{sending=false;render();}});
